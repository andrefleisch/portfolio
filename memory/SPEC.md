...
- Gap screenshot frames derive their height from each original image rather than a fixed frame ratio; images use `object-fit: contain`, automatic height, visible overflow, and no pixel alteration so the full iOS screen—including Dynamic Island, time, Wi-Fi, battery, and top border—remains visible. Center prominence comes from a wider grid column, not scale/hover transforms that could clip the image.
- The three Gap phones use an editorial supporting → focal → supporting hierarchy: equal columns, side frames at 92.5% width, the central Smart Recommendation frame at 100%, the center aligned slightly higher, and the right screen only slightly lower than the left. Labels use higher-contrast off-white while screenshots remain untouched.
